                    ...
                    <div class="col-md-12">
                        <div class="addEventForm mt-5">
                                ...
                                <div class="images-container" :class="{'justify-content-center' : files.length == 0}">
                                    ...
                                    <div class="no-banners-text" v-if="!loadingBanner && files.length == 0">
                                        <i class="fas fa-info-circle"></i> No Banners Found
                                    </div>
                                </div>
                                <hr>
                                <div class="d-flex justify-content-center">
                                    <div @click="inputFile()" class="dropzone-small image-box center-dropzone"  @drop.prevent="filesDropped" @dragover.prevent  @dragenter="isDragging = true" @dragleave="isDragging = false" @mouseleave="isDragging = false" :class="{'is-dragging': isDragging }" >
        ...
        methods: {
            getAllBanners(){
                ...
                .then( res => {
                    ...
                    this.photos = res.data.banners
                    this.photos.forEach( image => {
                        this.files.push( { id: image.id , url: '/storage/banners/'+ image.name , file: image , status: 'success' , server_file: image , uploadProgress: 100} )
                    })
            ...
            postImage(file, index)
            {
                ...
                fd.append('user_id', 0 )
                axios.post('/api/banner/store', fd, {
                    headers: {
